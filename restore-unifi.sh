...
MONGO_DB="unifi"
AUTOBACKUP_DIR="/var/lib/unifi/backup"
...
info "Restoring MongoDB database ${MONGO_DB}"
if [[ -d "$TMP_DIR/dump/$MONGO_DB" ]]; then
  mongorestore --drop "$TMP_DIR/dump/$MONGO_DB" || warn "mongorestore reported issues."
else
  warn "Mongo dump directory missing: $TMP_DIR/dump/$MONGO_DB"
fi

info "Restoring .unf autobackup files"
mkdir -p "$AUTOBACKUP_DIR"
if [[ -d "$TMP_DIR/backup" ]]; then
  cp -a "$TMP_DIR/backup"/* "$AUTOBACKUP_DIR"/ || warn "Failed to copy backup files."
else
  warn "Backup directory missing inside archive."
fi
